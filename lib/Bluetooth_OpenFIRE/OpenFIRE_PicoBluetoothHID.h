...
class OpenFIRE_PicoBluetoothHID_ {
public:
    OpenFIRE_PicoBluetoothHID_() {
    }
    ...
    // Usage: OpenFIRE_PicoBluetoothHID.setCanSendNowCB(std::bind(&kbd::onCanSendNow, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
    ...
    bool startHID(const bool hid_boot_device, const char *localName, const char *hidName, uint16_t hidClass, uint8_t hidSubclass, const uint8_t *hidDescriptor, uint16_t hidDescriptorSize);
...
public:
    bool end() {
        if (_running) {
            /// hci_power_control(HCI_POWER_OFF);  // messo nella gestione generale del bluetooth
        }
        _running = false;
        _needToSend = false;
        return true;
...
private:
    ...
    uint8_t _hid_service_buffer[1300]; //era 300 l'originale, ma troppi piccolo
