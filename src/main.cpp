...
#include <Arduino.h>
#ifdef _WIN32
    #include "SamcoEnhanced\SamcoEnhanced.h"
#else
    #include "SamcoEnhanced/SamcoEnhanced.h"
#endif
...
#ifdef _WIN32
#include "SamcoEnhanced\SamcoColours.h"
#include "SamcoEnhanced\SamcoPreferences.h"
#include "SamcoEnhanced\OpenFIREFeedback.h"
#else
#include "SamcoEnhanced/SamcoColours.h"
#include "SamcoEnhanced/SamcoPreferences.h"
#include "SamcoEnhanced/OpenFIREFeedback.h"
  ...
  void rp2040pwmIrq(void);
    ...
    //#include <hardware/pwm.h> // controllare
    //#include <hardware/irq.h> // controllare
    // declare PWM ISR
    //void rp2040pwmIrq(void);
    /////////////////////////////////#include "ESP32_PWM.h"
    ...
    void IRAM_ATTR esp32s3pwmIrq(void);
#endif
...
#ifdef USES_DISPLAY
#ifdef _WIN32
  #include "SamcoEnhanced\SamcoDisplay.h"
#else
    #include "SamcoEnhanced/SamcoDisplay.h"
#endif
...
//extern PicoBluetoothHID_OpenFIRE_ PicoBluetoothHID_OpenFIRE;
//extern SerialBT_OpenFIRE_ SerialBT_OpenFIRE;

//#ifdef OPENFIRE_BT_SPP
#ifndef OPENFIRE_BT_SPP
    
    
    #if defined(ARDUINO_RASPBERRY_PI_PICO_W) && defined(ENABLE_CLASSIC)
    //Stream* Serial_OpenFIRE_Stream;
    //HardwareSerial* Serial_OpenFIRE_HardwareSerial;
    //Adafruit_USBD_CDC* Serial_OpenFIRE_TinyUSBD_CDC;
    //Stream Serial_OpenFIRE_classe;
/*
    class Serial_OpenFIRE_classe_ : public  Stream {
        Serial_OpenFIRE_classe_() {}


    };

    Serial_OpenFIRE_classe_ Serial_OpenFIRE_classe;
*/
    /*
    ...
    */
    ...
    /////////////////////////--/////////////////////////////////extern Bluetooth_OpenFIRE_ Bluetooth_OpenFIRE;

    #include "Bluetooth_OpenFIRE.h"
    extern BT_OpenFIRE_ BT_OpenFIRE;
    ...
    #endif
    //////SerialBT_OpenFIRE_* Serial1_OpenFIRE = new SerialBT_OpenFIRE_;
    //delete Serial1_OpenFIRE;
/*
...
*/
    ...
    //Serial_OpenFIRE  =  & Serial;
    //#define Serial (*Serial_OpenFIRE)

    //Stream * Serial_OpenFIRE;
    //Serial_OpenFIRE  =  & SerialBT_OpenFIRE;


#endif
//#define Serial SerialBT_OpenFIRE
...
if (millis () - lastMessage >= MESSAGE_TIMEOUT)
    ...
    */
    //if (/*digitalRead(34) ||*/ 1) { //34 - WL_GPIO2 - digitalRead(34) NON FUNZIONA BLOCCA L'ACCESSO
    ...
    #define MILLIS_TIMEOUT  7000 //7 secondi
           
    unsigned long lastMillis = millis ();

    while ((millis () - lastMillis <= MILLIS_TIMEOUT) && (!USBDevice.mounted())) { yield(); }
    ////while ((!SerialBT_OpenFIRE) || (!PicoBluetoothHID_OpenFIRE) { yield(); }
    
       ////////////////////////////////////////////////////////////////////////////////////

     //bool confronto = (TinyUSBDevices.onBattery ? SerialBT_OpenFIRE : *Serial_OpenFIRE_TinyUSBD_CDC);

    if (USBDevice.mounted()) // && false)
    #else
    if (false)
    #endif
    {
        ...
        //Serial_OpenFIRE_HardwareSerial = & Serial;
        Serial_OpenFIRE_TinyUSBD_CDC = & Serial;
        
        
        //Serial_OppenFIRE_Tiny = & Serial;
        //Serial_OpenFIRE  =  & Serial;
        Serial_OpenFIRE_Stream->setTimeout(0);  // è uguale a scrivere <(*Serial_OpenFIRE).setTimeout(0);>
        (*Serial_OpenFIRE_Stream).setTimeout(0); // è uguale a scrivere <Serial_OpenFIRE->setTimeout(0);>
        //Serial_OpenFIRE_HardwareSerial->setTimeout(0);  // è uguale a scrivere <(*Serial_OpenFIRE).setTimeout(0);>
        //(*Serial_OpenFIRE_HardwareSerial).setTimeout(0); // è uguale a scrivere <Serial_OpenFIRE->setTimeout(0);>
        Serial_OpenFIRE_TinyUSBD_CDC->setTimeout(0);  // è uguale a scrivere <(*Serial_OpenFIRE).setTimeout(0);>
        (*Serial_OpenFIRE_TinyUSBD_CDC).setTimeout(0); // è uguale a scrivere <Serial_OpenFIRE->setTimeout(0);>
    ...
    /////////////else { // quando si vuole usare bluethoot - mettere un define magari o valutare alternative
        ...
        ////while ((millis () - lastMillis <= MILLIS_TIMEOUT) && (!USBDevice.mounted())) { yield(); }
        //while ((!SerialBT_OpenFIRE) || (!PicoBluetoothHID_OpenFIRE.connected())) { yield(); }

        while (!TinyUSBDevices.onBattery) { 
            yield();   // ?
        }
        ...
        ////////////////////////////////////--///////////////////////////////////////////////Serial_OpenFIRE_Stream = & Bluetooth_OpenFIRE; /////////****///////////
        
        
        ////////////////////////////////////////////////////////////////
        
        Serial_OpenFIRE_Stream = & BT_OpenFIRE.OpenFIRE_SerialBT;
        ...
        */
        ...
        // controlla che si collegato
        ////while(!USBDevice.mounted()) { yield(); }
        // imposta velocità e varie   
        ////Serial.begin(9600);
        ////Serial.setTimeout(0);
    ...
    if(bitRead(SamcoPreferences::pins.pCamSCL, 1) && bitRead(SamcoPreferences::pins.pCamSDA, 1)) {
        ...
        if(bitRead(SamcoPreferences::pins.pCamSCL, 0) && !bitRead(SamcoPreferences::pins.pCamSDA, 0)) {
            ...
            //Wire1.setSCL(SamcoPreferences::pins.pCamSCL);
                ...
                Wire1.setPins(SamcoPreferences::pins.pCamSDA, SamcoPreferences::pins.pCamSCL); // MODIFICATO
            #else //rp2040
                Wire1.setSDA(SamcoPreferences::pins.pCamSDA);
                Wire1.setSCL(SamcoPreferences::pins.pCamSCL);
            #endif
        ...
        if(bitRead(SamcoPreferences::pins.pCamSCL, 0) && !bitRead(SamcoPreferences::pins.pCamSDA, 0)) {
            ...
            #ifdef ARDUINO_ESP32S3_DEV              
                Wire.setPins(SamcoPreferences::pins.pCamSDA, SamcoPreferences::pins.pCamSCL); // MODIFICATO
            #else //rp2040
                Wire.setSDA(SamcoPreferences::pins.pCamSDA);
                Wire.setSCL(SamcoPreferences::pins.pCamSCL);
            #endif
...
*/
    ...
    #define TIMER_DIVIDER 80
    int32_t TimerTicks=50,000 //
    const int32_t TimerClock = APB_CLK_FREQ;   // il clock generale dei timer è di 80 mhz
*/
    //#define TIMER_DIVIDER 80
    //hw_timer_t *My_timer = NULL;
    My_timer = timerBegin(0, TIMER_DIVIDER, true);  // timer divider 80 - clock timer è di 80 mhz, vuol dire che Timer tick è 1mhz ovvero 1 microsecondo
    timerAttachInterrupt(My_timer, &esp32s3pwmIrq, true);
    timerAlarmWrite(My_timer, (uint64_t) 1000000 / frequencyHz, true); 
    ...
    //esp32s3EnablePWMTimer(0, frequencyHz); //slice_num 0 si intende qui il numero del timer -- ce ne sono 4
    //attachInterrupt(button1.PIN, esp32s3pwmIrq, FALLING);  // verificare
    //irq_set_exclusive_handler(PWM_IRQ_WRAP, esp32s3pwmIrq); // da togliere
    //irq_set_enabled(PWM_IRQ_WRAP, true); //da togliere
}

void esp32s3EnablePWMTimer(unsigned int slice_num, unsigned int frequency) //slice_num si intende qui il numero del timer -- ce ne sono 4
{
    
        
    /*
    noInterrupts(); // disattiva interrupt
    // critical, time-sensitive code here
    interrupts(); //attiva interupt
    // other code here

    attachInterrupt(digitalPinToInterrupt(interruptPin), ____, CHANGE);
    ...
    ledcSetup(PWM_CHANNEL, PWM_FREQ, PWM_RESOLUTION); //verificare

    // ledcAttachPin(uint8_t pin, uint8_t channel);
    ledcAttachPin(LED_OUTPUT_PIN, PWM_CHANNEL); //verificare
    ledcWrite(PWM_CHANNEL, dutyCycle); // verificare
    */
...
void IRAM_ATTR esp32s3pwmIrq(void) ///sistemare
{
    //pwm_hw->intr = 0xff;
    irPosUpdateTick = 1;
}
...
#if (defined(ARDUINO_ARCH_RP2040) || defined(ARDUINO_ESP32S3_DEV)) && defined(DUAL_CORE)
// Second core setup
...
void PrintResults()
{
    ...
    #else //rp2040
        //if(!Serial) {
        if (!(TinyUSBDevices.onBattery ? BT_OpenFIRE : *Serial_OpenFIRE_TinyUSBD_CDC)) {
        stateFlags |= StateFlagsDtrReset;
        return;
        }
    #endif                 
...
void PrintPreferences()
{
    ...
    #else //rp2040
        //if(!(stateFlags & StateFlag_PrintPreferences) || !Serial) {
        if(!(stateFlags & StateFlag_PrintPreferences) || !(TinyUSBDevices.onBattery ? BT_OpenFIRE : *Serial_OpenFIRE_TinyUSBD_CDC)) {    
        return;
        }
    #endif                 
...
void PrintDebugSerial()
{
    ...
    if(millis() - serialDbMs >= 1000) {
    // if(millis() - serialDbMs >= 1000 && Serial) {
    //if(millis() - serialDbMs >= 1000 && (TinyUSBDevices.onBattery ? SerialBT_OpenFIRE : *Serial_OpenFIRE_TinyUSBD_CDC)) {
        Serial.print("mode ");
        ...
        //SerialBT_OpenFIRE.print(" OpneFIRE Bluetooth: frame ");
        //SerialBT_OpenFIRE.print(frameCount);
        Serial.print(" OpneFIRE Bluetooth: frame ");
        Serial.print(frameCount);


        Serial.print("  OpneFIRE USB serial: frame  ");
        Serial.print(frameCount);

        Serial.print(", Mouse X,Y ");
        //Serial.print(conMoveXAxis);   // non lo trova
        Serial.print(",");
        //Serial.println(conMoveYAxis); // non lo trova
